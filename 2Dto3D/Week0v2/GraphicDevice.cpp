...
void FGraphicsDevice::Initialize(HWND hWindow) {
    ...
    CreateDepthStencilState(hWindow);
	CreateRasterizerState(hWindow);
}
...
void FGraphicsDevice::CreateFrameBuffer(HWND hWindow)
{
    // 스왑 체인으로부터 백 버퍼 텍스처 가져오기
    HRESULT hr = SwapChain->GetBuffer(0, __uuidof(ID3D11Texture2D), (void**)&FrameBuffer);
    if (FAILED(hr) || FrameBuffer == nullptr)
    {
        MessageBox(hWindow, L"Failed to get back buffer from swap chain!", L"Error", MB_ICONERROR | MB_OK);
        return;
    }
	...
	if (FAILED(hr)) {
		MessageBox(hWindow, L"Failed to create render target view!", L"Error", MB_ICONERROR | MB_OK);
		return;
	}
}
...
void FGraphicsDevice::CreateDepthStencilState(HWND hWindow)
{
    ...
    if (FAILED(hr)) {
        wchar_t errorMsg[256];
        swprintf_s(errorMsg, L"Failed to create depth stencil State! HRESULT: 0x%08X", hr);
        MessageBox(hWindow, errorMsg, L"Error", MB_ICONERROR | MB_OK);
        return;
    }
}

void FGraphicsDevice::ReleaseDepthStencilState()
{
...
void FGraphicsDevice::CreateRasterizerState(HWND hWindow)
{
...
void FGraphicsDevice::OnResize(HWND hWindow, float& aspectRatio) {
    DeviceContext->OMSetRenderTargets(0, nullptr, nullptr);
    ReleaseFrameBuffer();
    ReleaseDepthStencilBuffer();
    ...
    // SwapChain 크기 조정
    HRESULT hr;
    hr = SwapChain->ResizeBuffers(0, width, height, DXGI_FORMAT_B8G8R8A8_UNORM, 0);  // DXGI_FORMAT_B8G8R8A8_UNORM으로 시도
    if (FAILED(hr)) {
        MessageBox(hWindow, L"Failed to resize SwapChain Buffers!", L"Error", MB_ICONERROR | MB_OK);
        return;
    }
